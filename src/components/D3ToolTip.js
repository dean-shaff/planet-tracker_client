...
class D3ToolTip {
  ...
  show(d, i, node){
    ...
    let thisNode = node[i]
    let bounds = thisNode.getBoundingClientRect()
    let scrollTop  = document.documentElement.scrollTop || document.body.scrollTop
    let scrollLeft = document.documentElement.scrollLeft || document.body.scrollLeft
    let widthOffset = parseFloat(this.node.style("width"),10)*0.5
    let heightOffset = parseFloat(this.node.style("height"), 10)*1.1
    let nodeHeight = bounds.height
    let nodeWidth = bounds.width
